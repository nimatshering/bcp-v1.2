...
<div class="container py-4 mx-auto px-20">
    ...
    <div class="p-3 rounded font-sans w-full m-4">
      @include('backend.livewire.agency.climate-data.partials._climateProjectedExcelUpload')
    </div>

    <div class="mx-auto sm:px-6 lg:px-8">
      <div class="flex justify-between">
          <div class="flex flex-col p-2 my-2 font-semibold text-xl">
            Climate Projected Data
          </div>
          <div>
          ...
          <div class="-my-2 overflow-x-auto sm:-mx-6 lg:-mx-8">
              ...
              <table class="min-w-full divide-y divide-gray-200 table-auto">
                      ...
                      <tbody class="bg-white divide-y divide-gray-200">
                          ...
                          <td class="px-6 py-4 whitespace-nowrap">
                              <div class="text-sm text-gray-900">
                                  {!! optional(App\Models\Station::find($item->station_id))->name; !!}
                              </div>
                          </td>
                          ...
                          <td class="px-6 py-4 whitespace-nowrap">
                              <div class="text-sm text-gray-900">
                                  {!! optional(App\Models\ClimateModel::find($item->model_id))->name; !!}
                              </div>
                          </td>

                          <td class="px-6 py-4 whitespace-nowrap">
                              <div class="text-sm text-gray-900">
                                  {!! optional(App\Models\ClimateScenerio::find($item->scenerio_id))->name; !!}
                              </div>
                          </td>
          ...
          <x-slot name="content">
                ...
                <div class="mt-4">
                  ...
                  <x-jet-label for="station_id" value="{{ __('Select Station') }}" />
                    <select wire:model.defer="data.station_id" type="text" class="form-control mt-1 block w-full">
                ...
                <div class="mt-4">
                  <div class="col-span-6 sm:col-span-4">
                  <x-jet-label for="model_id" value="{{ __('Select Model') }}" />
                    <select wire:model.defer="data.model_id"  type="text" class="form-control mt-1 block w-full">
                        <option value="">Select Model</option>
                        @foreach ($modellist as $item)
                            <option value="{{ $item->id }}" >{{ $item->name }}</option>
                        @endforeach
                    </select>
                      <x-jet-input-error for="data.model_id" class="mt-2" />
                  </div>
                </div>
                ...
                <div class="mt-4">
                  <div class="col-span-6 sm:col-span-4">
                  <x-jet-label for="scenerio_id" value="{{ __('Select Scenerio') }}" />
                    <select wire:model.defer="data.scenerio_id"  type="text" class="form-control mt-1 block w-full">
                        <option value="">Select Scenerio</option>
                        @foreach ($sceneriolist as $item)
                            <option value="{{ $item->id }}" >{{ $item->name }}</option>
                        @endforeach
                    </select>
                      <x-jet-input-error for="data.scenerio_id" class="mt-2" />
                  </div>
                </div>
              ...
              <div class="mt-4">
                  <div class="col-span-6 sm:col-span-4">
                      <x-jet-label for="datepicker" value="{{ __('Observation/Reading Date') }}" />
                      <x-jet-input id="datepicker" type="text" class="mt-1 block w-full" wire:model.defer="data.date_of_reading" />
                      <x-jet-input-error for="data.date_of_reading" class="mt-2" />
                  </div>
              </div>
