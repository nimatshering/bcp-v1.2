 ...
 <div class="flex w-full">
   <form action="{{ route('agency.water.projected.xls.import') }}" method="POST" enctype="multipart/form-data">
    @csrf
    
    <label for="climatedata">Import Excel Data</label>
    ...
    <select name ="station" id="station" class="form-control">
      ...
      <select name ="scenerio" id="scenerio" class="form-control">
        <option>Choose Parameter</option>
        @foreach($sceneriolist as $item)
            <option value="{{ $item->id }}">{{ $item->name }}</option>
        @endforeach
      </select>
        <input type="file" name="waterdatafile" class="form-control">
...
<div class="flex flex-1 bg-green-700 hover:bg-green-600 px-4 py-2 rounded my-2 w-4/12 text-white font-bold text-xs uppercase">
        <a href="/uploads/dataImportFiles/template_water_projected.xlsx" >Download Format for life import</a>
</div>